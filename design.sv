...
parameter full_cycle             = 8'd255 //256 // timp incare butonul nu mai ia valori dupa apasarea butonului si dureaza pana cand rosu se schimba in verde semafor masina
)(
...
//starting the cycle
   if(rst_n)    
      cycle_counter <= 0;
    else if(push_button_counter == full_cycle - 1)  //sem car red // counter starts
      cycle_counter <= cycle_counter + 1'b1;
    if (cycle_counter == full_cycle)
      cycle_counter <= 0;
 
//tranzitii 
   if(cycle_counter > car_green_reg) //sem car green 
		semafor_masini <= 'b100;
    
	if(cycle_counter > car_yellow_reg) //sem car yellow  
		semafor_masini <= 'b010;
		
	if(cycle_counter > car_red_reg) //sem car green
		semafor_pietoni <= 'b10 ;
		semafor_masini <= 'b001;
...
always@(posedge clk or negedge rst_n)
begin
  pushed_button <= buton_pietoni_apasat;
end
  assign buton_apasat = buton_pietoni_apasat & ~pushed_button;
