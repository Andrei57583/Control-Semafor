...
`ifndef __intrare_sequence_senzor_lumina
`define __intrare_sequence_senzor_lumina

//se declara o clasa care genereaza o secventa de date
class secventa_intrare_senzor_lumina extends uvm_sequence #(tranzactie_intrare);
  
  //noul tip de data (secventa) se adauga la baza de date UVM
  `uvm_object_utils(secventa_intrare_senzor_lumina)
  ...
  function new(string name="secventa_intrare_senzor_lumina");
    super.new(name);
  endfunction
    
  function void post_randomize();
    $display("secventa_intrare_senzor_lumina: Marimea sirului de tranzactii=%0d", numarul_de_tranzactii);
   endfunction
  ...
  virtual task body();
    ...
    //`endif;
    `uvm_info("secventa_intrare_senzor_lumina", $sformatf("A inceput secventa cu dimensiunea de %-2d elemente", numarul_de_tranzactii), UVM_NONE)

    parsing_day(10);
    `uvm_info("secventa_intrare_senzor_lumina", $sformatf("S-au generat toate cele %0d tranzactii", numarul_de_tranzactii), UVM_LOW)
  endtask
 ...
 task parsing_day(int durata_ora);     
  ...
  endtask
   ...
   task nop(bit senzor, bit[4:0] ora );
      ...
      `ifdef DEBUG
      `uvm_info("secventa_intrare_senzor_lumina", $sformatf("La timpul %0t s-a generat elementul %0d cu informatiile:\n ", $time, i), UVM_LOW)
