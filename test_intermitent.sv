`ifndef __test_intermitent
`define __test_intermitent

`include "uvm_macros.svh"
//se includ fisierele la care testul trebuie sa aiba acces
`include "mediu_verificare.sv"
`include "secventa_apb_intermitent.sv"
`include "secventa_intrare_intermitent.sv"

class test_intermitent extends uvm_test;
  `uvm_component_utils(test_intermitent)
  
  
  //se declara constructorul testului
  function new(string name = "test_intermitent", uvm_component parent=null);
  ...
  secventa_intrare_intermitent intrare_seq;
  secventa_apb_intermitent apb_seq;
...
//uvm_config_db#(virtual output_interface_dut)::set(this, "mediu_de_verificare.agent_apb_din_mediu.*", "apb_interface_dut",vif_apb_dut);
    ...
    apb_seq = secventa_apb_intermitent::type_id::create("apb_seq");
    apb_seq.randomize();

    intrare_seq = secventa_intrare_intermitent::type_id::create("intrare_seq");
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    apply_reset();
    `uvm_info("test_intermitent", "real execution begins", UVM_NONE);
