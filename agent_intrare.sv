...
`ifndef __intrare_agent
`define __intrare_agent

//se includ dependentele folosite de agent
typedef class monitor_intrare;//aceasta este o pre-definire a tipului de data monitor_agent_semafoare folosita pentru a evita erorile aparute in urma faptului ca monitorul importa colectorul de coverage, si colectorul de coverage importa monitorul; explicatia in engleza: this is a forward type definition used to solve cross dependency between monitor and coverage class
`include "tranzactie_intrare.sv"
`include "coverage_intrare.sv"
`include "driver_agent_intrare.sv"
`include "monitor_intrare.sv"


class agent_intrare extends uvm_agent;
  
  `uvm_component_utils (agent_intrare)//se adauga agentul la baza de date a acestui proiect; de acolo, acelasi agent se va prelua ulterior spre a putea fi folosit
  ...
  driver_agent_intrare driver_agent_intrare_inst0;
  
  monitor_intrare  monitor_intrare_inst0;
  
  uvm_sequencer #(tranzactie_intrare) sequencer_agent_intrare_inst0;
  ...
  uvm_analysis_port #(tranzactie_intrare) de_la_monitor_intrare; //de_la_monitor_agent_semafoare;
  ...
  function new (string name = "agent_intrare", uvm_component parent = null);
      super.new (name, parent);
    de_la_monitor_intrare = new("de_la_monitor_intrare", this);
  endfunction 
  ...
  virtual function void build_phase (uvm_phase phase);
    ...
    monitor_intrare_inst0 = monitor_intrare::type_id::create ("monitor_intrare_inst0", this);
    ...
    if (is_active==1) begin
      sequencer_agent_intrare_inst0 = uvm_sequencer#(tranzactie_intrare)::type_id::create ("sequencer_agent_intrare_inst0", this);
      driver_agent_intrare_inst0 = driver_agent_intrare::type_id::create ("driver_agent_intrare_inst0", this);
    end
  ...
  virtual function void connect_phase (uvm_phase phase);
    ...
    de_la_monitor_intrare = monitor_intrare_inst0.port_date_monitor_intrare;
    ...
    if (is_active==1)begin
      driver_agent_intrare_inst0.seq_item_port.connect (sequencer_agent_intrare_inst0.seq_item_export);
    end
