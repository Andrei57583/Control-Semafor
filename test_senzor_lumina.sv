`ifndef __test_senzor_lumina
`define __test_senzor_lumina

`include "uvm_macros.svh"
//se includ fisierele la care testul trebuie sa aiba acces
`include "mediu_verificare.sv"
`include "secventa_apb_senzor_lumina.sv"
`include "secventa_intrare_senzor_lumina.sv"

class test_senzor_lumina extends uvm_test;
  `uvm_component_utils(test_senzor_lumina)
  
  
  //se declara constructorul testului
  function new(string name = "test_senzor_lumina", uvm_component parent=null);
  ...
  secventa_intrare_senzor_lumina intrare_seq;
  secventa_apb_senzor_lumina apb_seq;
...
//uvm_config_db#(virtual output_interface_dut)::set(this, "mediu_de_verificare.agent_apb_din_mediu.*", "apb_interface_dut",vif_apb_dut);
    ...
    apb_seq = secventa_apb_senzor_lumina::type_id::create("apb_seq");
    apb_seq.randomize();

    intrare_seq = secventa_intrare_senzor_lumina::type_id::create("intrare_seq");
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    apply_reset();
    `uvm_info("test_senzor_lumina", "real execution begins", UVM_NONE);
  ...
  virtual task apply_reset();
    ...
    vif_apb_dut.penable  <= 0;
    vif_apb_dut.psel     <= 0;
    vif_intrare_dut.buton_pietoni     <= 0;
    vif_intrare_dut.senzor_lumina     <= 0;
    vif_intrare_dut.ora_curenta     <= 0;
    `ifdef DEBUG
