...
`ifndef __intrare_sequence_button_pedestrian
`define __intrare_sequence_button_pedestrian

//se declara o clasa care genereaza o secventa de date
class secventa_intrare_buton_pieton extends uvm_sequence #(tranzactie_intrare);
  
  //noul tip de data (secventa) se adauga la baza de date UVM
  `uvm_object_utils(secventa_intrare_buton_pieton)
  ...
  function new(string name="secventa_intrare_buton_pieton");
    super.new(name);
  endfunction
    
  function void post_randomize();
    $display("secventa_intrare_buton_pieton: Marimea sirului de tranzactii=%0d", numarul_de_tranzactii);
   endfunction
  ...
  virtual task body();
    ...
    //`endif;
    `uvm_info("secventa_intrare_buton_pieton", $sformatf("A inceput secventa cu dimensiunea de %-2d elemente", numarul_de_tranzactii), UVM_NONE)
...
/*
   task nop(bit senzor, bit[4:0] ora );
      ...
      `ifdef DEBUG
      `uvm_info("secventa_intrare_buton_pieton", $sformatf("La timpul %0t s-a generat elementul %0d cu informatiile:\n ", $time, i), UVM_LOW)
