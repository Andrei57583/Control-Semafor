...
class monitor_output extends uvm_monitor;
 ...
 // coverage_output colector_coverage_output; 
  ...
  function new(string name = "monitor_output", uvm_component parent = null);
    ...
    port_date_monitor_output = new("port_date_monitor_output",this);
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual output_interface_dut)::get(this, "", "output_interface_dut", interfata_monitor_output))
        `uvm_fatal("MONITOR_output", "Nu s-a putut accesa interfata monitorului")
  endfunction
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    forever begin
      ...
      @(negedge interfata_monitor_output.clk); 
      //preiau datele de pe interfata de iesire a DUT-ului (interfata_semafoare)
      starea_preluata_a_output.masini = interfata_monitor_output.semafor_masini;
      starea_preluata_a_output.pietoni = interfata_monitor_output.semafor_pietoni;

      starea_preluata_a_output.buzzer = interfata_monitor_output.buzzer_pietoni;
      
      starea_preluata_a_output.lampa = interfata_monitor_output.lampa;



      aux_tr_output = starea_preluata_a_output.copy();//nu vreau sa folosesc pointerul starea_preluata_a_output pentru a trimite datele, deoarece continutul acestuia se schimba, iar scoreboardul va citi alte date 
      ...
      port_date_monitor_output.write(aux_tr_output); 
      `uvm_info("MONITOR_output", $sformatf("S-a receptionat tranzactia cu informatiile:"), UVM_NONE)
