...
`ifndef __intrare_sequence_intermitent
`define __intrare_sequence_intermitent

//se declara o clasa care genereaza o secventa de date
class secventa_intrare_intermitent extends uvm_sequence #(tranzactie_intrare);
  
  //noul tip de data (secventa) se adauga la baza de date UVM
  `uvm_object_utils(secventa_intrare_intermitent)
  ...
  function new(string name="secventa_intrare_intermitent");
    super.new(name);
  endfunction
    
  function void post_randomize();
    $display("secventa_intrare_intermitent: Marimea sirului de tranzactii=%0d", numarul_de_tranzactii);
   endfunction
  ...
  virtual task body();
    ...
    //`endif;
    `uvm_info("secventa_intrare_intermitent", $sformatf("A inceput secventa cu dimensiunea de %-2d elemente", numarul_de_tranzactii), UVM_NONE)

    parsing_day(10);
    `uvm_info("secventa_intrare_intermitent", $sformatf("S-au generat toate cele %0d tranzactii", numarul_de_tranzactii), UVM_LOW)
  endtask

 task parsing_day(int durata_ora);     
 //task parsing_day();     

  bit regim_intermitent;
  ...
  for (int i=0;i<24;i++)begin
    ...
    `uvm_info("DEBUG_ORAR", $sformatf("Ora: %0d | Intermitent: %b", i, regim_intermitent), UVM_LOW)
  ...
  endtask
   ...
   task nop(bit senzor, bit[4:0] ora );
      ...
      `ifdef DEBUG
      `uvm_info("secventa_intrare_intermitent", $sformatf("La timpul %0t s-a generat elementul %0d cu informatiile:\n ", $time, i), UVM_LOW)
